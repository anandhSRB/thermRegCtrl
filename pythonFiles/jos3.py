...
class JOS3():
    ...
    def _run(self, dtime=60, passive=False, output=True):
        ...
        if self._hr is not None:
            hr = self._hr
        if self._wallFlux is not None:
            print('Setting CFD wall heat fluxes')                            
            wallFlux = self._wallFlux
        ...
        if wallFlux is None:
            ...
            print('Setting sensible heat loss')
            seatFlux = np.zeros((17,))
            if self._seatHeater is not None:
                seatFlux = self._seatHeat*np.array([0,0,0,0,0.473,0,0,0,0,0,0,0.269,0,0,0.257,0,0])
            shlsk = wallFlux*self._bsa + seatFlux
            to = tsk - shlsk * r_t /self._bsa
    ...
    @property
    def wallHeatFlux(self):
        """Getter        

        Returns:
            wallFlux: numpy.ndarray(17,)
                      heatflux on the cloth/skin from CFD
        """
        return self._wallFlux
    @wallHeatFlux.setter
    def wallHeatFlux(self,inp):
        self._wallFlux = _to17array(inp)
    ...
    def rt(self):
        ...
        Returns:
            resistance between skin and wall: numpy.ndarray(17,)                      
        """
        return self._rt
    @rt.setter
    def rt(self,inp):
        self._rt = _to17array(inp)
